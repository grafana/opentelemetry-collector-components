...
# 20210617094655-main-deadbeef # using commit date and changeset when no changes were made
# 20210617094655-main-modified # using current date and the string 'modified' instead of changeset.
...
# crop the BRANCH string because the max length for dockertag variable is 70 symbols
DOCKER_TAG=$(echo "${ISO_TIMESTAMP}-${BRANCH:0:15}-${GIT_COMMIT_SHORT}" | tr "/" "_")
echo -n "${DOCKER_TAG}"
